...
Toets of oost-west-verkeer tussen netwerkdelen daadwerkelijk **langs** de firewall gaat, of er onderling
omheen beweegt. Vergelijk de **routeringstabellen** en flow-/sessielogs met de bedoelde segmentatie. Verkeer
dat niet langs de firewall komt, kan de firewall niet zien of tegenhouden — dat is het pad waarover lateral
movement na een besmetting verloopt.

Veelvoorkomende bevinding: een kantoor-/Citrix-omgeving die met een **L4**-firewall (poort/protocol) van de
servers is gescheiden in plaats van **L7** (applicatie/identiteit). Toets dit; ga niet uit van "het is
gescheiden".
...
Is er **TLS-decryptie/IPS** op de uitgaande werkplek-egress? Zonder decryptie blijft veel moderne malware-
en C2-verkeer (versleuteld over 443) onzichtbaar. Weeg dit af tegen privacy: leg uitzonderingen vast en
betrek de juiste gremia (privacy, medezeggenschap).
...
- exposure van de management-plane en de **MGT-VLAN**-locatie (is het beheer-VLAN bereikbaar vanaf de werkplek?);
- poortbeveiliging en **802.1X**, en of routering verkeer langs de firewall dwingt.
...
slordig ontwerp; versmal op basis van App-ID, in eigen tempo, in dialoog met de ketenpartners.

### 7. Onbeheerde paden
Let op werkplekken of VPN-/extern-toegangsroutes die wél routering naar het datacenter kennen maar niet in
beheer zijn. Stel de bewegingsvrijheid vast met een gerichte scan vanaf zo'n device; neem het niet aan.
...
1. **Nu, laag risico:** MFA op alle beheeraccounts; OoB-beheer; besluit over TLS-decryptie.
